...
namespace Mapbox.Examples
{
	public class AstronautMouseController : MonoBehaviour
	{
		...
		{
			if (cam == null)
				cam = GameManager.Instance.CameraObject[0].GetComponent<Camera>();
		...
		Vector3 nextPos;
		void MoveToNextPlace()		//이부분을 제어-> GPS기반을 통해 (oldGPS-newGPS).normalized -> 해당 좌표 이동.
		{
        ...
        {
			if (other.CompareTag("Enemy"))			//여기서 씬 전환.
			{
