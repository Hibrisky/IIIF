...
public class PieceGenerator : MonoBehaviour {
	...
	private void Start () {
		float sizeX = piecePrefab.GetComponent<Renderer>().sharedMaterial.mainTexture.width;
		float sizeY = piecePrefab.GetComponent<Renderer>().sharedMaterial.mainTexture.height;

		float uvWidth = 1.0f / colCount;
		float uvHeight = 1.0f / rowCount;

		Vector3 offset = Vector3.zero;//잘린 이미지의 사이즈를 받아와야한다...
		offset.x = -1f;//-(float) colCount / sizeX;//2.0f + .475f;
		//offset.y = -(float) rowCount / sizeY;//2.0f + .475f;
		float startX = offset.x;

		pieceMeshArray = new Mesh[rowCount, colCount];
		topConnector = new bool[rowCount, colCount];
		rightConnector = new bool[rowCount, colCount];
		...
		for (int j = 0; j < colCount; j++) {
			for (int i = 0; i < rowCount; i++) {
				...
				pieceGameObject.transform.localScale = new Vector3 (3.5f, 3.5f, 3.5f);
				...
				//ATTENTION: If the puzzle looks warped, research what version of unity you're on and change the uvs index accordingly
				// 0, 2, 3, 1 should work for Unity 2020+
